...
**B. Dynamic lv0 patch (attempted, causes GLOD):** Replace the hardcoded literal with a code cave that calls `get_reference_clock()` and computes `tb_clk = ref_clk / (ref_clk / 79800000)` at boot. Uses LFSR table entries 32-39 as a code cave for 8 PPC64 instructions. See `patch_lv0_tb_clk.py`. This approach is technically correct but causes a Green Light of Death on patched retail systems for unknown reasons. Not currently usable.
...
### 11.2 Dynamic Patch (Attempted, GLOD)

`patch_lv0_tb_clk.py` replaces the hardcoded literal with a branch to a code cave that calls `get_reference_clock()` and computes `tb_clk = ref_clk / (ref_clk / 79800000)` dynamically. The code cave is placed in LFSR table entries 32-39 (RefDiv values 32-39 are never used in practice). This would make lv0 automatically report the correct tb_clk at any overclock frequency.

The code cave assembles 8 PPC64 instructions:
```
...
b    <return>                # -> bl config_string_append_entry
```

This approach is technically correct but causes a Green Light of Death on patched retail systems. The root cause is unknown, it may be related to the `get_reference_clock()` call happening in a context where syscon SPI communication is not yet available, or a code signing/integrity check that detects the modification. Not currently usable.
...
Setting NVS 0x3130 to 0x00 enables the override system. Any NVS entry left as all-FF uses the ROM default. When an entry has any non-FF byte, the firmware uses the NVS data for that entire entry. Revert: `w 3130 FF`
...
## Confirmed Stable Profile: "Tight-3"

First confirmed working XDR timing + arbitration modification on PS3 hardware. Seven timing parameters tightened across 3 MIC registers plus read-biased arbitration.

```
w 315A 62 83 01 5A D6 B0    # MIC_TRCD_PCHG: tRDP=1, tWRP-1
w 3160 51 40                # MIC_CMD_DUR: tRC=21
w 3162 71 60 02 10 00 00    # MIC_CMD_SPC: tDeltaWR=8
w 3170 14 00 00 00 00 00 00 00  # MIC_Que_BurstSize: ReadQ=2, WriteQ=16
```

| Parameter | Stock (C-bin) | Tight-3 | Change | Stock ns (2.5ns) | Tight-3 ns (2.162ns) |
|-----------|---------------|---------|--------|------------------|----------------------|
| tRC | 24 | 21 | -12.5% | 60.0 | 45.4 |
| tRAS/tRP | 0x70 | 0x40 | -3 steps | -- | -- |
| tRDP | 4 | 1 | -75% | 10.0 | 2.2 |
| tWRP | ~12 | ~11 | -1 step | 30.0 | 23.8 |
| tDeltaWR | 10 | 8 | -20% | 25.0 | 17.3 |
| ReadQ_BurstSize | 4 | 2 | -50% | n/a | n/a |
| WriteQ_BurstSize | 12 | 16 | +33% | n/a | n/a |

Stability verified with 30+ minutes of gaming (Just Cause 2, other titles).

### Hard Limits Found (CECHA00 with Elpida C-bin)

| Parameter | Floor | Failure Mode |
|-----------|-------|-------------|
| tRCD-R | 7 (stock) | 5 = XDR link fail (0xb0002001) |
| tRCD-W | 3 (stock) | Already at minimum |
| tRC | 21 | 20 = intermittent data corruption |
| tRAS/tRP | 0x40 | 0x30 = intermittent lv0 auth fail |
| tRDP | 1 | MIC hardware minimum |
| tWRP | 0x83 | 0x82 = lv0 not found, 0x81 = XDR link fail |
| tDeltaRW | 9 (stock) | 8 = lv0 auth fail |
| tDeltaWR | 8 | Rule 2 hard floor: must be > tRCD_R (7) |
| MIC_DF_CTL | Stock only | Any change crashes (XIO-coupled) |

## Complete 46-Entry NVS Map

### Legend

- **TUNED**: Optimized value in Tight-3 profile
...
| 15 | 0x315A | 6 | 62 84 05 5A D6 B0 | MIC_TRCD_PCHG | **TUNED** | tRDP=1, tWRP-1. See bitfield table below. |
| 16 | 0x3160 | 2 | 5D 70 | MIC_CMD_DUR | **TUNED** | tRC=21. See bitfield table below. |
| 17 | 0x3162 | 6 | 71 80 02 10 00 00 | MIC_CMD_SPC | **TUNED** | tDeltaWR=8. See bitfield table below. |
...
| 21 | 0x3170 | 8 | 00 00 00 00 00 00 00 00 | MIC_QUE_BURSTSIZE | **TUNED** | ReadQ=2, WriteQ=16. |
...
Entries 35-36 (XIO_PLL_0) accept wide value ranges without crashing. Entry 37 upper byte (0xFC) is critical. Entries 38-39 crash at 0x00 but work at stock. A memory bandwidth benchmark is needed to determine if PLL changes affect actual clock frequency.
...
1. Build SPU DMA throughput benchmark to measure bandwidth impact of BurstSize, TM_Threshold, and per-channel byte changes.
2. Probe entry 40 bit-by-bit to map which bits control PTC enable vs PTC configuration.
3. Test entry 44 range more granularly to understand the per-lane function.
4. Possibly reverse engineer lv0 further to understand how the MIC is actuall programmed.
...
1. Set NVS 0x3130 to 0x00 (enables the XDR config override system)
2. Write timing bytes to entries in NVS 0x3140-0x31AF
3. Any entry left as all-FF uses the ROM default from firmware table at 0x402AC
4. On boot, sub_1E548 builds a 112-byte config block from NVS + ROM defaults
5. Cell requests the block via SERV_SETCFG (SID 18, cmd 0, sub 0x10)
6. lv0 programs the MIC (Memory Interface Controller) registers inside Cell
7. MIC controls all XDR DRAM timing and scheduling
...
Revert: `w 3130 FF` (disables everything, guaranteed boot)
...
|------|-------|--------|-----------|
| - | Enable override (0x3130=0x00) | `w 3130 00` | BOOT OK |
...
## Confirmed Stable Profile: "Tight-2"

First confirmed working XDR timing modification on PS3 hardware. Tested on COK-001 CECHA00
with Elpida XDR DRAM, Evilnat 4.92 CFW. Stability verified with 30 minutes of Just Cause 2.

```
# Apply complete profile (3 writes):
w 315A 62 83 02 5A D6 B0
w 3160 51 40
w 3162 71 60 02 10 00 00
```
...
| tRDP | 0x05 | 0x02 | -3 steps | MIC_Trcd_Pchg byte 2 (0x315C) | 15 |
| tWRP (partial) | 0x84 | 0x83 | -1 step | MIC_Trcd_Pchg byte 1 (0x315B) | 15 |
| tDeltaWR | 10 (0x80) | 8 (0x60) | -20.0% | MIC_Cmd_Spc byte 1 (0x3163) | 17 |

**6 timing parameters tightened across 3 MIC registers.**

**Impact:** Row cycle time reduced (faster bank recycling), row active and precharge
times reduced (faster row operations), read-to-precharge delay reduced (rows close faster
after reads), write recovery tightened (writes complete sooner), write-to-read turnaround
reduced (less dead bus time on every W->R switch). All improvements affect sustained memory
bandwidth and latency under mixed read/write workloads (i.e., all games).

**Prerequisites:**
- NVS 0x3130 = 0x00 (XDR override enabled)

**Turnaround encoding (MIC_Cmd_Spc byte 0/1):**
- Byte 0 upper nibble = tDeltaRW - 2 (stock: 0x7 = 9)
- Byte 0 lower nibble = critical (likely tCC/tPP-D). 0x1 is required, 0x0 crashes.
- Byte 1 upper nibble = tDeltaWR - 2 (stock: 0x8 = 10, tuned: 0x6 = 8)
- Byte 3 (0x10) = critical protocol timing (likely tRFC/tRR). DO NOT reduce.

**Hard limits found on this unit (CECHA00 with Elpida XDR):**
- tRCD-R: 7 is floor (5 crashes - XDR link won't train)
- tRCD-W: 3 is floor (already at minimum)
- tRC: 21 is floor (20 causes intermittent data corruption during boot)
- tRAS/tRP: 0x40 is floor (0x30 causes intermittent lv0 auth fail)
- tRDP: 0x02 is stable (0x01 not yet tested - could try)
- tWRP: 0x83 is floor (0x82 = lv0 not found, 0x81 = XDR link fail, 0x80 = hangs)
- tDeltaRW: 9 is floor (8 crashes - lv0 auth fail from corrupted reads)
- tDeltaWR: 8 is floor (minimum allowed by HIG Rule 2: must be > tRCD-R of 7)
- MIC_DF_CTL byte1: DO NOT CHANGE (0x96->0x86 breaks DMA/IO path, not XDR link)
...
```
# Enable XDR override system (required once)
w 3130 00

# Apply "Tight-2" profile (confirmed stable, game tested)
w 315A 62 83 02 5A D6 B0
w 3160 51 40
w 3162 71 60 02 10 00 00

# Revert single entries to ROM default
w 315A FF FF FF FF FF FF        # revert MIC_Trcd_Pchg (tRDP, tWRP)
w 3160 FF FF                    # revert MIC_Cmd_Dur (tRC, tRAS/tRP)
w 3162 FF FF FF FF FF FF        # revert MIC_Cmd_Spc (turnarounds)

# Revert: disable entire override system
w 3130 FF
```
...
- Your value: 7 (stock C-bin). Tested 5 -> XDR link fail (0xb0002001).
- Rambus spec note: tRCD-R only takes odd values {3,5,7,9} per architecture.
- Why 5 crashes: The Elpida C-bin DRAM core physically cannot complete row sensing in 5 tCYCLEs at 466.7 MHz. At stock 400 MHz, 5 tCYCLEs = 12.5 ns. At 466.7 MHz, 5 tCYCLEs = 10.7 ns. The A-bin minimum at 3.2G is 5*2.5 = 12.5 ns. Your actual minimum would be 12.5/2.143 = 5.83, rounded up to 7 (next odd).
...
3. **MIC_DF_Ctl is already Sony-tuned.** The firmware default (0x0A963D60) differs from the HIG sample (0x0A543CE0), meaning Sony already customized the dataflow timing (tQTD/tQRD/tERD) for PS3-specific XDR chips. Changing this register requires understanding the exact XDR DRAM part binning.

4. **The XDR present flag (0x3130)** must be set to non-0xFF for any XDR timing overrides to take effect. On stock consoles this may be 0xFF, meaning lv0 uses its own defaults.

5. **All timing changes are loaded at boot** - the syscon serves the config to lv0 during the POR sequence. Changes take effect on next power cycle.

### Example: Tightening XDR Latency

To reduce XDR memory latency at stock 400 MHz:

1. Set 0x3130 to 0x00 (enable XDR config override)
2. Write tighter timing values to 0x314E (MIC_Cmd_Dur) - reduce tRC, tRAS, tRP
3. Write tighter values to 0x3154 (MIC_Cmd_Spc) - reduce tDeltaRW, tDeltaWR

The ROM defaults are conservative. The actual XDR DRAM chips (Elpida/Samsung) may support tighter timings than what Sony programmed, just as DDR RAM often runs below its rated speed in stock configurations.
